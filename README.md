...
- 任务运行时菜单栏图标旁展示进度状态.
- 工作台, Finder 或 Services 选择文件时, 空闲状态会载入选择, 运行中会加入等待队列.
...
- Finder Sync 优先使用 App Group handoff 文件传递大量选择, 避免文件多时 URL 过长.
- Finder handoff 默认限制文件数量和 payload 大小, 使用私有目录和文件权限, App Group 不可用时回退开发环境临时目录.
...
- 已支持 Finder Sync App Group handoff 文件传参, 并保留旧 URL query 入口作为兼容兜底.
