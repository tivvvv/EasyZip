...
- 任务运行时菜单栏图标旁展示进度状态.
- Finder 或 Services 选择文件时, 有可用输出目录会直接启动或加入等待队列, 无可用输出目录会打开工作台等待选择.
- 任务队列支持顺序执行, 当前任务结束后自动启动下一个等待任务.
- 退出应用时会拦截运行中任务, 确认后先取消任务再退出.
- 任务完成后发送 macOS 系统通知.
- 工作台和菜单栏状态面板展示当前会话任务队列, 支持顺序执行, 重试, 取消, 清理和定位输出.
- Finder Sync 扩展提供 `使用易压缩进行压缩` 和 `使用易压缩进行解压` 右键菜单.
- Finder 右键任务需要密码, 冲突决策或输出目录时会自动打开工作台继续处理.
- Finder 后台任务失败时会发送系统通知, 避免失败信息只停留在未打开的工作台.
- Finder Sync 通过 LaunchServices 将所选文件交给主应用获取沙盒授权, App Group handoff 负责匹配压缩或解压动作.
- Finder handoff 默认限制文件数量和 payload 大小, 使用私有目录和文件权限. LaunchServices 调用失败时回退 bookmark URL, App Group 不可用时少量选择通过 URL 中的 security scoped bookmark 回退, bookmark 不完整或超出 URL 限制时会打开工作台提示重新选择.
...
- 设置页支持失效默认输出目录提示, 工作台会提示重新选择输出目录.
- 默认输出目录使用 security scoped bookmark 持久化沙盒写入权限.
- 设置页支持一键恢复默认设置.
- 诊断页展示安装后状态汇总, 可快速打开扩展设置, 通知设置, 登录项设置, 工作台并重启 Finder.
...
- 已新增环境诊断页, 可检查安装位置, Finder 扩展包, 沙盒授权, Finder Sync, App Group, 通知权限, RAR 命令, zstd 命令, 默认输出目录和签名状态.
- 环境诊断页已提供安装后快速修复入口, 支持打开扩展设置, 通知设置, 登录项设置, 工作台和重启 Finder.
- 已新增设置页和设置持久化, 工作台会读取默认任务选项并同步设置变更提示.
- 已支持失效默认输出目录检测和默认设置恢复.
- 已新增当前会话任务队列, 每个任务记录独立状态, 进度, 结果和重试快照, 并支持等待任务顺序执行.
- 已支持 Finder Sync 通过 LaunchServices 传递文件访问授权, App Group handoff 匹配动作模式, 并保留 URL security scoped bookmark 和旧 item query 入口作为兼容兜底.
- 已对 Finder handoff 增加权限, 容量, security scoped bookmark 和启动清理保护.
- 已新增 App 和 Finder Sync extension 的 sandbox entitlements 模板, 打包时默认写入 `group.com.tiv.easyzip`.
...
- 已提供正式 `.app` 包结构构建脚本.
- 已提供发布构建脚本, 可生成 zip, dmg, SHA256 校验文件和构建摘要, 并会解包复检 zip.
- 已提供 DMG 打包和完整性检查脚本, 发布包会包含应用和 Applications 入口, 并支持可选 DMG 签名和签名校验.
- 已提供可选公证脚本, 可配合 Developer ID 签名和 Apple notarytool 使用, 发布流程可自动 staple app 和 dmg.
- 已提供产物完整性检查脚本, 可独立校验 `.app` 结构, Info.plist, Finder Sync extension, App Group entitlements, 签名一致性, Hardened Runtime 和公证票据.
...
CODE_SIGN_IDENTITY="Developer ID Application: Example" RELEASE_REQUIRE_DEVELOPER_ID=1 Scripts/release_build.sh
CODE_SIGN_IDENTITY="Developer ID Application: Example" RELEASE_NOTARIZE=1 NOTARY_KEYCHAIN_PROFILE=easyzip Scripts/release_build.sh
...
默认 App Group 为 `group.com.tiv.easyzip`, 可通过 `APP_GROUP_IDENTIFIER` 覆盖.
当 `CODE_SIGN_IDENTITY` 不是 `-` 时, 发布构建会要求 Hardened Runtime, 并默认用同一身份签名和校验 DMG.
如需跳过 DMG 签名, 可设置 `DMG_CODE_SIGN_IDENTITY=-`.
设置 `RELEASE_REQUIRE_DEVELOPER_ID=1` 会强制检查 Developer ID Application 签名.
设置 `RELEASE_NOTARIZE=1` 后, 发布构建会先提交临时 app zip 公证并 staple app, 再生成最终 zip 和 dmg, 最后提交 dmg 公证并 staple dmg.
...
推荐正式发布使用 `RELEASE_NOTARIZE=1 Scripts/release_build.sh`, 让脚本自动完成 app 和 dmg 公证.
也可以单独运行 `Scripts/notarize_release.sh dist/release/EasyZip-0.1.0-1.dmg` 提交已有 dmg.
