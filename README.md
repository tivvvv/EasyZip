...
第一期聚焦 `.zip` 和 `.7z` 格式, 支持归档预览, 解压和压缩. 核心能力独立封装在
`EasyZipCore` Swift Package 中, 图形界面通过 `EasyZipApp` 提供任务式拖拽工作台.
...
- 解压模式支持选择输出目录, 冲突处理策略和单归档预览.
- 任务执行时展示字节级进度, 并提供取消和在 Finder 中定位输出目录.

## 核心设计

- 高内聚核心模块, 归档领域模型, 引擎协议, 服务入口和安全校验分层清晰.
- 低耦合引擎接入, 后续新增 RAR, tar, gzip, xz 等格式时优先扩展 `ArchiveEngine`.
...
- 已支持 `.zip` 和 `.7z` 的创建, 列表读取和解压.
- 已支持冲突重命名, 字节级进度, 取消检查和加密归档识别.
- 已新增 `EasyZipApp` macOS 图形界面入口.
